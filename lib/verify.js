...
function runVerify({ cwd, hookPath } = {}) {
  ...
  if (!checks.rulesLoaded || !fs.existsSync(hook)) {
    return { ok: false, mode, ruleCount, fingerprint: fp.fingerprint, checks, details, coverage: emptyCoverage };
  }
  ...
  try {
    ...
    return { ok, mode, ruleCount, fingerprint: fp.fingerprint, checks, details, coverage };
  } finally {
