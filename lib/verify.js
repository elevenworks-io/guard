...
function runVerify({ cwd, hookPath } = {}) {
  ...
  if (checks.rulesLoaded) {
    ...
    checks.uniqueIds = dupes.length === 0;
    if (!checks.uniqueIds) {
      details.push({
  ...
  try {
    ...
    // blocksSecret fließt in `ok` ein, aber vorher wurde dafür nie etwas nach
    // `details` gepusht — fehlt path.dotenv (Regel gelöscht/entschärft), sah
    // der Nutzer NUR grüne Zeilen gefolgt von "✕ Verifikation fehlgeschlagen.
    // Ursache oben." — ohne dass oben je eine Ursache stand. Ein
    // undiagnostizierbarer Dauerzustand (jeder SessionStart meldet dann
    // "FEHLGESCHLAGEN", ohne Weg zur Diagnose).
    ...
    details.push({
      ...
      info: checks.blocksSecret
        ? ".env → blockiert (path.dotenv)"
        : "Regel „path.dotenv\" fehlt oder greift nicht — guard verify verlangt sie als Positivkontrolle",
    });
