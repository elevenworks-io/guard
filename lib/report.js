...
function countRules(rules) {
  return (rules.blockedPaths?.length || 0) + (rules.blockedCommands?.length || 0) + (rules.piiPatterns?.length || 0);
}
function buildReport({ auditLines, rules, now }) {
  ...
  lines.push(`- **${blocked.length} blockiert**, ${warned.length} gewarnt, ${allowed.length} erlaubt`);
  lines.push("");
